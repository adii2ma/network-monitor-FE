...
export default function UpdatePage() {
  ...
  const handleAction = async (action: "add" | "delete") => {
    ...
    try {
      let url = `/api/${action}`;
      if (action === "add") {
        url += `?ip=${encodeURIComponent(ip)}&location=${encodeURIComponent(location)}`;
      } else {
      ...
      if (res.ok) {
        if (action === "add") {
          setMessage(`IP ${ip} added successfully with location: ${location}!`);
          setIp("");
          setLocation("");
        } else {
        ...
        <div className="flex flex-col gap-4 justify-center items-center mb-6">
          <div className="flex flex-col sm:flex-row gap-4 w-full">
            <input
              ...
              className="border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-200 flex-1"
            />
            <input
              type="text"
